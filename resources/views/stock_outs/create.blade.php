...
@section('topbar-title', 'Input Stok Keluar')
...
<div class="page-header">
    <div>
        <h1 class="page-title">Input Stok Keluar</h1>
        <p class="page-subtitle">Kurangi stok produk karena penjualan atau alasan lain</p>
    </div>
    <a href="{{ route('stock_outs.index') }}" class="btn btn-sm btn-outline-secondary">
...
<div class="card-qs p-4" style="max-width:700px;">
    <form action="{{ route('stock_outs.store') }}" method="POST">
        ...
        <div class="mb-4">
            <label class="form-label fw-600">Keterangan / Alasan</label>
            <textarea name="notes" class="form-control @error('notes') is-invalid @enderror"
                      rows="2" placeholder="contoh: Penjualan harian, barang rusak, dll">{{ old('notes') }}</textarea>
            @error('notes')<div class="invalid-feedback d-block">{{ $message }}</div>@enderror
        </div>

        <button type="submit" class="btn btn-qs-primary w-100 py-2" style="background:linear-gradient(135deg, #dc2626, #ef4444); box-shadow: 0 4px 14px rgba(220,38,38,.3);">
